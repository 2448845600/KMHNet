...
class GWNET(AbstractTrafficStateModel):
    def __init__(self, config, data_feature):
        ...
        self.kr_adj = config.get('kr', False)
        self.poi_adj = config.get('poi', False)
        self.ns_adj = config.get('ns', False)
        self.dist_adj = config.get('dist', False)
        ...
        if self.kr_adj:
            kr_support = torch.load('raw_data/SZBus/kr_adj_mat.pth').to(device=self.device, dtype=torch.float)
            self.supports.append(kr_support)  
            self._logger.info('supports add kr_support, shape: {}, sum: {}'.format(kr_support.shape, kr_support.sum()))
        if self.poi_adj:
            poi_support = np.loadtxt('raw_data/SZBus/poi_adj_mat.csv')
            poi_support = torch.from_numpy(poi_support).to(device=self.device, dtype=torch.float)
            self.supports.append(poi_support)  
            self._logger.info('supports add poi_support, shape: {}, sum: {}'.format(poi_support.shape, poi_support.sum()))
        if self.ns_adj:
            ns_support = np.loadtxt('raw_data/SZBus/ns_adj_mat.csv')
            ns_support = torch.from_numpy(ns_support).to(device=self.device, dtype=torch.float)
            self.supports.append(ns_support)
            self._logger.info('supports add ns_support, shape: {}, sum: {}'.format(ns_support.shape, ns_support.sum()))
        if self.dist_adj:
            self.supports.extend([torch.tensor(i).to(self.device) for i in self.adj_mx])
        ...
        if self.gcn_bool and self.addaptadj:
            # if self.aptinit is None:
                ...
                self.nodevec2 = nn.Parameter(torch.randn(10, self.num_nodes).to(self.device),
                                             requires_grad=True).to(self.device)
                self.supports_len += 1
                self._logger.info('supports add aptadj')
    ...
    def cal_adj(self, adjtype):
        print('origin adj_mx, shape: {}, max: {}, min:{}, mean: {}'.format(self.adj_mx.shape, self.adj_mx.max(), self.adj_mx.min(), self.adj_mx.mean()))
        if adjtype == "scalap":
